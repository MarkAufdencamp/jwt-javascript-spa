...
- **Docker:** Used to package the application with an Apache web server for consistent production-ready environments.
- **Kamal:** Used for automated deployment of the Dockerized application to remote servers.
...
- **Centralized Configuration:** A global `CONFIG` object (defined in `config.js`) manages environment-specific settings like the base API URL across all modules.
...
- **Local Serving:** Static files should be served via a local web server (e.g., Python's `http.server` or Node's `http-server`) to handle `fetch` requests and ensure `config.js` is correctly loaded.
- **Docker Compose:** Provides a standardized, containerized local development environment, including volume mounting for rapid iteration.
